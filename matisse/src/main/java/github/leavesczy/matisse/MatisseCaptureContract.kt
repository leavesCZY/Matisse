 ...
 * Activity 使用 `Theme.Matisse.Capture`（透明窗），不强制竖屏。拍照并成功读取结果时返回
 * [MediaResource]；用户取消、相机不可用、权限被拒绝或结果无效时返回 null。此流程不请求媒体读取权限；
 * 宿主声明 `CAMERA` 后会按需申请，存储权限和照片存储位置由 [MatisseCapture.captureStrategy] 决定。
 */
