 ...
 *
 * 确认选择时返回非空的 [MediaResource] 列表；Activity 未以成功结果结束、结果 Intent 缺失或结果列表为空时
 * 返回 null。权限被拒或媒体加载失败不会自动结束选择器，用户返回后结果为 null。
 * 若配置了 [Matisse.captureStrategy]，选择器内拍照成功后不会结束流程，新照片会插入“全部”相册列表首位且不自动选中；
 * 用户确认选择后才返回结果。宿主应用需要提前在 Manifest 中声明 [Matisse.mediaType] 对应的媒体读取权限，
 * 权限申请由选择器完成；具体权限规则参见 [Matisse]。选择器界面使用 `Theme.Matisse`，固定为竖屏。
 */
